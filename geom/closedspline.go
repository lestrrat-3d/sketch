...
func (sp *ClosedSpline) Eval(t float64) (float64, float64) {
	return EvalPeriodicCubicBSpline(controlCoords(sp.Control), t)
}
...
func (sp *ClosedSpline) Polyline(segments int) [][2]float64 {
	return SamplePeriodicCubicBSpline(controlCoords(sp.Control), segments)
}
...
func SamplePeriodicCubicBSpline(ctrl [][2]float64, segments int) [][2]float64 {
	if segments < 3 {
		segments = 3
	}
	pts := make([][2]float64, segments+1)
	for i := 0; i <= segments; i++ {
		x, y := EvalPeriodicCubicBSpline(ctrl, float64(i)/float64(segments))
		pts[i] = [2]float64{x, y}
	}
	return pts
}
