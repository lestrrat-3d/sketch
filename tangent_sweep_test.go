...
func TestTangentLineArcOutOfSweepRejected(t *testing.T) {
	...
	start := s.CreatePoint(10, 5) // 0°
	end := s.CreatePoint(5, 10)   // 90°
...
func TestTangentLineArcInSweepSatisfied(t *testing.T) {
	...
	center := s.CreatePoint(5, 5)
	s.Fix(center)
	start := s.CreatePoint(1, 1) // ~225°
	end := s.CreatePoint(9, 1)   // ~315°, sweep straddles 270°
	arc := s.CreateArc(center, start, end)
	s.AddConstraint(sketch.NewTangent(s.CreateLine(a, b), arc))
...
func TestTangentCirclesArcOutOfSweepRejected(t *testing.T) {
	...
	o := s.CreatePoint(0, 0)
	s.Fix(o)
	circ := s.CreateCircle(o, 8)
	s.AddConstraint(sketch.NewRadius(circ, 8))

	center := s.CreatePoint(10, 0)
	start := s.CreatePoint(12, 0) // 0°
	end := s.CreatePoint(10, 2)   // 90°; contact toward the circle (180°) is out of sweep
	arc := s.CreateArc(center, start, end)
	s.Fix(center)
	s.Fix(start)
	s.Fix(end)
...
func TestTangentLineArcAtEndpointBoundary(t *testing.T) {
	...
	s.Fix(center)
	start := s.CreatePoint(5, 0) // 0°
	end := s.CreatePoint(0, 5)   // 90°
	arc := s.CreateArc(center, start, end)
	s.Fix(start)
	s.Fix(end)

	// Vertical line through the arc's start point: tangent at the 0° endpoint.
	top := s.CreatePoint(5, 8)
	line := s.CreateLine(start, top)
...
func TestTangentArcSweepRoundTrip(t *testing.T) {
	...
	s.Fix(b)
	center := s.CreatePoint(5, 5)
	start := s.CreatePoint(10, 5)
	end := s.CreatePoint(5, 10)
	arc := s.CreateArc(center, start, end)
	s.Fix(center)
	s.Fix(start)
	s.Fix(end)
...
func TestTangentLineThroughArcEndpointMustBeTangent(t *testing.T) {
	s := newSketch(t)
	center := s.CreatePoint(0, 0)
	start := s.CreatePoint(5, 0) // 0deg, radius 5
	end := s.CreatePoint(0, 5)   // 90deg
	arc := s.CreateArc(center, start, end)
	s.Fix(center)
	s.Fix(start)
	s.Fix(end)
...
func TestTangentConcentricCirclesRejected(t *testing.T) {
	...
	o := s.CreatePoint(0, 0)
	s.Fix(o)
	c1 := s.CreateCircle(o, 3)
	c2 := s.CreateCircle(o, 5) // same center
...
func TestTangentArcCheckConstraintNoPanic(t *testing.T) {
	...
	require.NotPanics(t, func() {
		_ = s.CheckConstraint(sketch.NewTangent(s.CreateLine(a, b), arc))
	})
}
...
func TestTangentArcNearDegenerateLineNoPanic(t *testing.T) {
	...
	center := s.CreatePoint(0, 0)
	s.Fix(center)
	start := s.CreatePoint(-3, -4)
	end := s.CreatePoint(3, -4)
	arc := s.CreateArc(center, start, end)
...
func TestTangentSharedArcsRespectInternal(t *testing.T) {
	build := func(internal bool) error {
		...
		s1 := s.CreatePoint(0, 5)
		p := s.CreatePoint(5, 0) // shared contact, between the centers -> external
		a1 := s.CreateArc(c1, s1, p)
		c2 := s.CreatePoint(10, 0)
		e2 := s.CreatePoint(10, 5)
		a2 := s.CreateArc(c2, p, e2) // shares p
...
func TestTangentArcFeasibleSeededOutOfSweep(t *testing.T) {
	...
	start := s.CreatePoint(8, 5) // 0deg
	end := s.CreatePoint(5, 8)   // 90deg
	arc := s.CreateArc(center, start, end)
	s.AddConstraint(sketch.NewTangent(s.CreateLine(a, b), arc))
