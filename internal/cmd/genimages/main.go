...
// commitHash returns a short VCS revision to stamp into the watermark: the
// build-embedded revision when available (go run/test stamps it), else a live
// `git rev-parse`, else "unknown". The in-sync test normalizes this out so a new
// commit does not by itself invalidate the committed images.
func commitHash() string {
	if info, ok := debug.ReadBuildInfo(); ok {
		for _, s := range info.Settings {
			if s.Key == "vcs.revision" && s.Value != "" {
				return shortHash(s.Value)
			}
		}
	}
...
// watermark is the provenance string stamped on every framed hero.
var watermark = "generated by github.com/lestrrat-3d/sketch@" + commitHash()
