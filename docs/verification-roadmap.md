...
|---|---|---|
| ~~**Profile/region engine**~~ — *shipped* | `Profiles()` now runs a planar arrangement (`geom.Regions`): bare-crossing subdivision, nested loops/holes + containment, winding/orientation, net area, and **self-intersection detection** (a malformed region reports `Valid=false` and gates `Verify().Trustworthy()`), plus a degeneracy (coincident-edge / near-tangent) uncertainty signal. The oracle no longer blesses a self-intersecting or unresolvable profile. **Splines now participate** (`geom.Spline` is a `Curve`: sampled to a polyline like arcs, sampled fragment area, and a spline-only same-source crossing test so a self-crossing cubic is flagged `SelfIntersecting` even when the crossing lands on a sample vertex). **Ellipse/elliptical-arc fragment area is now EXACT** (`chordEllipseCorrection` = ½·rx·ry·(Δφ − sin Δφ), the elliptical analog of the circular-segment correction, rotation- and translation-invariant), so an ellipse-bounded region's reported `Area` (and the validity that gates `Trustworthy()`) is no longer a sampled approximation — only splines remain sampled (no closed form). Open follow-ups: an analytic (non-sampled) arrangement (near-tangent/degeneracy topology), spline fragment area. | XL |
...
| **Solver robustness & export fidelity** *(rank-margin advisory shipped)* | Shipped the oracle-core slice as an **advisory** signal: `Verify()` reports `RankMargin` — the multiplicative distance of the constraint Jacobian's closest rank decision from the hard 1e-9 pivot threshold (`rankAnalysisOf` in `solver.go`), so a near-threshold value flags that the DOF/redundancy verdict was decided by a fragile pivot (e.g. a point pinned onto two near-parallel lines). It is **advisory only and does NOT gate `Trustworthy()`**: the raw pivots are scale-dependent (angle-constraint derivatives grow with line length), so the same construction at different scales gives different margins — it is not a unit-invariant condition number and must not be thresholded as pass/fail. It does not rediscover exact redundancy (already caught by rank). Remaining: a **scale-invariant** (dimensionally-normalized) conditioning gate (needs per-var-kind column scaling), analytic Jacobian rows / equation decomposition (perf/accuracy), world-space DXF, display-unit metadata on exports. | L–M |
