...
| **Constraint/dimension parity** *(in progress)* | Shipped: H/V between points, generalized midpoint, radius/diameter & concentric on arcs (batch 1); entity Fix/ground (`FixEntity`), symmetric lines & circles (batch 2). Remaining: arc symmetry (needs endpoint swap+mirror), unified Equal across line/arc and arc-length dim (both need a continuous-sweep aux var — deferred), coincident point-to-entity (the point-on-X pieces exist), tangent-edge distance, angle-quadrant selection, ordinate/baseline/chained dims. | S–L |
