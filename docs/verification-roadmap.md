...
| **Constraint/dimension parity** *(in progress)* | Shipped: H/V between points, generalized midpoint, radius/diameter & concentric on arcs (batch 1); entity Fix/ground (`FixEntity`), symmetric lines & circles (batch 2); arc-length dimension (`NewArcLength`, continuous-sweep aux variable — batch 3). Remaining: arc symmetry (endpoint swap+mirror), line↔arc Equal (compose arc-length with line length), point-on-arc (sweep-confined), driven arc-length, tangent-edge distance, angle-quadrant selection, ordinate/baseline/chained dims. | S–L |
