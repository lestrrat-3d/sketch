...
func TestRankMarginNearSingularFlagged(t *testing.T) {
	...
	// it is structurally Trustworthy — but the two on-line constraints are nearly
	// linearly dependent, so the advisory RankMargin reports the near-threshold
	// rank decision (a hint to the agent), without flipping the canonical verdict
	// (the raw margin is scale-dependent, so it must not gate trust).
	s := sketch.New()
	o1 := s.AddPoint(0, 0)
	e1 := s.AddPoint(1, 0)
	o2 := s.AddPoint(0, 0)
	...
	require.True(t, rep.Trustworthy(), "structurally clean; the advisory margin does not gate the verdict")
}
