...
| `revision.go` | `Sketch.Revision()`: a **fingerprint** (FNV-1a over the whole var vector + the entity set/types/construction flags + per-entity defining points + per-entity **structural state**), not a counter — compare for **equality only, never order**. Derived from state rather than bumped per mutating method **on purpose**: there is no bump-site list to forget, so a new mutation path cannot silently leave the revision stale. It must cover everything `Profiles()` reads — coordinates *and* topology *and* the construction flag (toggling construction changes the region set while every coordinate stays put) *and* entity data the solver cannot move. **Load-bearing rule: hash what `buildProfiles` CONSUMES, never a proxy for it.** Entity fields (`Line.Start`, …) are exported, so a defining point can be rewired to a `*Point` of another sketch or to a removed handle whose id a live point has since inherited; `buildProfiles` follows the pointer (`Point.Geometry()` → that point's own `s.vars`) and shares one `geom.Point` per distinct `*Point`. So each defining point is hashed as a triple — a first-seen **sharing ordinal** (never the pointer address: not deterministic across runs), its **id**, and the **coordinates the pointer resolves to** (`math.Float64bits`, since a foreign point's coordinates are not in this sketch's var vector at all). The var vector only covers what the solver owns, so anything else `buildProfiles` reads goes through `entityStructuralState` — a switch with a case per entity type (today only `*NURBS`: `degree`/`knots`/`weights`, hashing slice lengths + `math.Float64bits`). **A new entity type carrying non-var structural data MUST get a case there**, or a change to it leaves the revision identical and a stale `Profile` reads fresh. |
