...
do), then require (1) the spliced polylines to meet ONLY at those points — no
crossing interior to a segment of each (`polylinesMeetOnlyAtVertices`), since a
handled pair's sampled crossings are never recorded, so a leftover one has no vertex
and the face walk fuses the regions on either side (the round-2 bug); and (2) the
four chord departures at each injected point to ALTERNATE between the sources
(`portsCross`), in the same rotation order `buildGraph` sorts by — meeting at a point
is not crossing at it. Both parts are **threshold-free**: no tolerance, no chord
bound, no crossing-angle floor. Measured over ~24k circle/circle configurations
(radius ratios, the whole transverse band including both near-tangent ends, five
angles, three scales, `spt` 3–90) plus multi-circle and arc/arc scenes: **zero
blessed-but-wrong**, and zero cases where a correct lifted arrangement was refused —
the certificate is an exact predictor of when injection is safe, not a conservative
approximation to it. Certification reaches 100% of the transverse band from `spt=16`
(`Sketch.Profiles` samples at 256). The **fallback is the SAMPLED path, never a
degeneracy**: an uncertified pair is left unhandled exactly as before, so it keeps
the correct sampled topology with `TExact=false`, and no arrangement that was
blessed before is refused now. Both halves are needed — dropping the port
alternation blessed 110 wrong topologies in the same sweep.
