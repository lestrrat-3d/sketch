...
| `svg.go` / `png.go` / `dxf.go` / `json.go` / `json_world.go` | Exporters / serialization. `png.go` is a stdlib-only rasterizer (`image/png`) so agents/tools that read raster images can sanity-check sketches; visually equivalent to the SVG output (PNG annotation is a follow-up — SVG is the annotated target). `dxf.go` emits length fields in the sketch's **display length unit** (via the `units` library — angles/ratios/knots stay raw) with a matching `$INSUNITS`/`$MEASUREMENT` + `$EXTMIN`/`$EXTMAX` header, so a CAD importer reads the drawing at the right scale (metric output is unchanged). Coordinates are plane-**local** by default; `DXF(WithWorldSpace(true))` places geometry in 3D world coordinates via the plane frame — LINE/SPLINE/ELLIPSE in WCS, CIRCLE/ARC/LWPOLYLINE in the entity OCS (arbitrary-axis algorithm from the plane normal) + extrusion, arc angles recomputed in the OCS. **`SVG`/`PNG`/`DXF` all refuse a sketch whose *reported bounding box* is not finite** — checked once via `bbox.finite()` after `renderBounds`/`bounds()`, returning the shared `ErrNonFiniteGeometry` sentinel. That is a narrower promise than "no NaN/Inf coordinate reaches the output": `bounds()` samples a curve at a fixed 32 points, so it is a **coarse, non-exhaustive probe**, and three distinct gaps below it are reachable today through `CreateNURBS` (whose non-decreasing check compares knots with `<` — false against NaN in both directions — so an INTERIOR NaN knot passes silently; the clamping check uses `!=` and does reject a NaN in a clamped position, but tests no interior knot for finiteness). (1) A NaN whose affected knot span is narrower than the 32-sample spacing is invisible to `bounds()` — the exporter returns a nil error and emits geometry that is silently wrong rather than corrupt. (2) `bounds()`'s sampling density is hardcoded at 32 and never tracks the renderer's own `cfg.arcSegments` (default 64, caller-settable via `WithArcSegments`), so the guard can be coarser than what actually gets drawn: at a high enough `WithArcSegments`, `SVG` returns a nil error while its path data contains literal `NaN` tokens. (3) `DXF` writes a NURBS knot vector **verbatim** (`pairf(40, k)`) into the native `SPLINE` record, a path `bounds()` never samples at all, so a NaN knot reaches the file at any sampling density. Closing gap (3), and tightening (1)/(2), needs a finiteness check in `CreateNURBS` itself — out of scope for this guard, which only bounds what a healthy sketch's reported box promises. Without the guard at all, `PNG` was the sharpest case: its `w <= 0 -> 1` clamp never fires against NaN, so a NaN width reached `image.NewNRGBA` and panicked, a documented non-panicking call crashing from an unchecked value. `JSON` already refuses non-finite geometry via `encoding/json`'s own NaN rejection, so it needed no change. `ErrNonFiniteGeometry` is exported so a future `Verify` condition can reuse the same sentinel for this fact. `json_world.go` is the v2 `World`/`Plane` serialization + the `kind`-discriminator preflight. |
