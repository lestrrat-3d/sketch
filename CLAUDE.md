...
construction; a contact mapped onto an EXISTING sample vertex does not, because
`cutSite` decides that in the source's PARAMETER (within `segEps` of a segment
boundary) and a parameter that close still leaves a POSITION gap orders of magnitude
above round-off — certifying it published the vertex's own sample fraction `i/n` as an
exact bound for a crossing that is not there, with no second net, since certification
exempts the pair from the taint passes. (3) The four chord departures at each injected
...
`spt=16` (`Sketch.Profiles` samples at 256); an endpoint contact and a within-round-off
contact are refused at EVERY density, by construction rather than by coarseness.
...
reconciliation bound is deliberately the tight one. Regressions:
`TestAnalyticCurveCrossingNeverBlessedWrong` (blessed ⇒ correct over the
...
or a closed-form crossing parameter, never a sample fraction) and
