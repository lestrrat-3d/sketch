...
**A pair the kernel never classified withdraws the same way** (`nearMisses`,
`noteChordNearMiss`): an ellipse/conic/spline/NURBS pair rests entirely on the chords,
and a chord can hide a crossing inside the band its own curve deviates from it (measured
per segment in `densify`). Where two such chords approach within `dev_i+dev_j` and no
sampled contact of that pair explains the approach, the two sources are joined and the
component loses its exactness — the sampled twin of a refused analytic crossing, and the
reason a certified circle pair beside an ellipse that clips it cannot publish the fused
map as exact. The bound is what keeps this narrow: a curve merely passing NEAR another
(a hole inside a disk) is farther off than either chord can hide, so it is never
recorded, and the scene keeps its exactness.
...
its exactness) and `TestSampledOnlyPairMissingCrossingNotBlessedExact` (an ellipse
clipping one of two certified circles, with the parked-ellipse control).
