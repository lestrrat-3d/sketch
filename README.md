...
each one that is clear of the geometry and of the other names. Where position
alone cannot say which vertex a name belongs to, the name is underlined and
carries an arrow to its own point, so a sketch reasoned about as "A to B" says
which point is which. The gallery also renders
